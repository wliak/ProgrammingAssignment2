...
# To validate:
# mtx <- makeCacheMatrix()
# mtx$set(matrix(c(0,3,3,0), 2, 2)) or an alternative invertible matrix up to the memory capacity of the host computer.
# mtx$get(m)
# cacheSolve(mtx)
# cacheSolve(mtx)
...
makeCacheMatrix <- function(x = matrix()) {
    local_mtx <- NULL                                         ## Initialize the local mtx to NULL so we can tell when cacheSolve has run at least once.  
    ...
    set_cache_m <- function(local_mtx) cache_m <<- local_mtx    ## Create function to set the value of cache_m in cache to the value of local_mtx passed in the call to '$set_cache_m.        
